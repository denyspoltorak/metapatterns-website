...
*Open Host Service* \[[DDD]({{< relref "../appendices/books-referenced.md#ddd" >}})\] is an enterprise pattern about providing functionality for use by other components \(hence the name\) through publishing a stable interface, known as a *Published Language* \[[DDD]({{< relref "../appendices/books-referenced.md#ddd" >}})\]\. As the subject service should be able to change, and as its clients may differ in their needs, requiring multiple *Published Languages*, an *Open Host Service* usually involves an *adapter* for every *Published Language* it provides – just like *Separated Presentation* does for supported *UI*s\.
