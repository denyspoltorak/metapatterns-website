...
- A [*Data Grid*]({{< relref "#data-grid-of-space-based-architecture-sba-replicated-cache-distributed-cache" >}}) is a distributed in\-memory database that replicates data among many instances of various services\.
...
*Stamp Coupling* \[[SAHP]({{< relref "../appendices/books-referenced.md#sahp" >}})\] happens when a single data structure passes through an entire [*Pipeline*]({{< relref "../basic-metapatterns/pipeline.md" >}}), with separate fields of the data structure targeting individual processing steps \(as administrative offices used to rubber stamp the paper documents which passed through them\)\.
